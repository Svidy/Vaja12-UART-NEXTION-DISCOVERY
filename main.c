...
int main(void)
{
  ...
  HAL_UART_Receive_DMA(&hadc1, myRxData, 6);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  HAL_ADC_Stop(&hadc1);
	  reading=AdcValue;
	  reading/=23; //pretvorba: 0 → 0, 4192 → 180, količnik je približno 23!
	  myTxData[0]=(reading/100)+48;
	  myTxData[1]=((reading/10)%10)+48;
	  myTxData[2]=(reading%10)+48;
	  myTxData[3]=0xff;
	  myTxData[4]=0xff;
	  myTxData[5]=0xff;
...
{
UNUSED(huart4);
if (myRxData[0]==0xaa) HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);
if (myRxData[0]==0xbb) HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);
}
